...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (msg.includes('already registered') || msg.includes('409')) setError('An account with this email already exists.');
      else if (msg.includes('fetch') || msg.includes('NetworkError')) setError('Cannot reach the backend. Make sure the server is running on port 8000.');
      else setError(msg || 'Failed to create account. Please try again.');
    } finally {
      setLoading(false);
    }
  };
